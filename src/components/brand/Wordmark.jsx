 ...
 *
 * The lockup is also the splash's shared element, so it exposes the two parts
 * that reveal separately by data attribute rather than by prop: the mark, which
 * arrives on its own, and the box the wordmark is wiped in through. Neither has
 * any styling of its own for that — the splash writes an opacity and a
 * clip-path to them and clears both afterwards, so the header's copy is
 * untouched before and identical after.
 *
...
export function Wordmark({ className, style }) {
  return (
    ...
    >
      ...
      {/*
       * The wipe box. `inset-0` makes its border box the lockup's own, so the
       * clip-path the provider animates is expressed in the same percentages as
       * the glyph insets below: closed at `right: 68.4%` the visible strip ends
       * at 31.6% — the wordmark's left edge, a hair right of the mark — so the
       * first frame of the reveal emerges from the mark rather than from
       * nowhere. Left, top and bottom never move: the wipe is horizontal by
       * construction, not by easing.
       */}
      <span data-lockup-word className="absolute inset-0">
        <svg
